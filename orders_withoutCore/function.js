				...
				$('#OrderDialogTable').last().append(
					'<tr status="Load" class='+DoorTRColor+' id="OTR'+guidSmall()+'" idDoor='+obj.Doors[i].id+'>'+
						...
						'<td type=Cost><button onclick=" OrderCostDialogLoad(this)" style=" '+((AutorizeType==4 || AutorizeType==3) ? "display:none; " : "")+'; '+ColorButtonCost+' " title="Стоимость">...</button>'+
							...
							'</span>'+
						'</td>'+
						"<td><button style='"+(AutorizeType!=1 & DoorTRColor!="Start"? "display:none; ":"")+"' onclick='OrederSpecificLoad(this)' title='Спецификация'>...</button></td>"+
...
function OrderPrlCalcAddEditRow(RowID){
	...
	$.post(
		"propertes/prop.php",
		{"Method":"PropPrlLoad", "DoorType":RowEl.find("td[type=Name]").text(), "Step":Step[j]},
		function(data){
			var o=jQuery.parseJSON(data); var i=0;
			var Sum=0;
			//Размеры двери
			while(o.Size[i]!=null){
				...
				if((S & o.Size[i].S==1) || (!S & o.Size[i].S==2)) CalcOk=false;
				if(RowEl.find("td[type=S]").text()!="")
					switch(o.Size[i].SSign)
					{
						case "1": if(parseFloat(Sv)>=parseFloat(o.Size[i].SVal)) CalcOk=false; break;
						case "2": if(parseFloat(Sv)<=parseFloat(o.Size[i].SVal)) CalcOk=false; break;
						case "3": if(parseFloat(Sv)!=parseFloat(o.Size[i].SVal)) CalcOk=false; break;
					};
				if(o.Size[i].Open!=""  & (o.Size[i].Open!=Open)) CalcOk=false;
				if(!Framuga & o.Size[i].Framug==1) CalcOk=false;

				if(CalcOk) 
					Sum=parseFloat(o.Size[i].Sum);
			...
			if(o.Constructor.Frame==1)
			{
				...
				if((workwindowch=="true" & workwindownoframe=="false") || (stvorkawindowch=="true" & stvorkawindownoframe=="false") || (framugawindowch=="true" & framugawindownoframe=="false"))
					if(o.Constructor.FrameCount==1)
					{
						...
						PetlyaCount+=(stvorkawindowch=="true" & stvorkawindownoframe=="false") ? 1 :0;
						PetlyaCount+=(framugawindowch=="true" & framugawindownoframe=="false") ? 1 :0;
						Sum+=parseFloat( o.Constructor.FrameSum)*PetlyaCount;
					}
					else
					{
						Sum+=parseFloat(o.Constructor.FrameSum);
					};
			};
			//Доводчик
			if(o.Constructor.Dovod==1 )
			{
				if(RowEl.find("td[type=Dovod]").text().toUpperCase()=="ДА" & o.Constructor.DovodPreparation!=1)
					Sum+=parseFloat(o.Constructor.DovodSum);
				if(o.Constructor.DovodPreparation==1)
					if(RowEl.find("td[type=Dovod]").text().toUpperCase()=="НЕТ, ПОДГОТОВКА" || RowEl.find("td[type=Dovod]").text().toUpperCase()=="ДА")
						Sum+=parseFloat(o.Constructor.DovodSum);
			};
			...
			if(o.Constructor.Nalichnik==1)
				if(RowEl.find("td[type=Nalichnik]").text().toUpperCase().indexOf("ДА")>-1)
					Sum+=parseFloat(o.Constructor.NalichnikSum);
			//Окно
			if(o.Constructor.Window==1)
			{
				...
				if(WindowCount!=0)
					switch (o.Constructor.WindowCount){
						case "1"://Зависит от кол-ва
							//Если больше
							switch (o.Constructor.WindowMore){
								case null:
									Sum+=WindowCount*parseFloat(o.Constructor.WindowSum);
									break;
								default:
									if(WindowCount>parseInt(o.Constructor.WindowMore))
										Sum+=(WindowCount-parseInt(o.Constructor.WindowMore))*parseFloat(o.Constructor.WindowSum);
									break;
							}
							break;
						default://Не зависит от кол-ва
							Sum+=parseFloat(o.Constructor.WindowSum);
							break;
					};
			};
			//Фрамуга
			if(o.Constructor.Framuga==1 & RowEl.find("td[type=Construct] span framugach").text()=="true")
				Sum+=parseFloat(o.Constructor.FramugaSum);
			//Навесы
			if(o.Constructor.Petlya==1)
			{
				var PetlyaSum=parseFloat(o.Constructor.PetlyaSum);
				...
				if(PetlyaCount!=0)
					if(o.Constructor.PetlyaCount==1)//Зависит от кол-ва
					{
						if(o.Constructor.PetlyaMore==null)//Не заполненно поле болшльше
							PetlyaSum=PetlyaCount*parseFloat(o.Constructor.PetlyaSum);
						//Заполненно поле больше, т.е. расчитываем по разнице
						if(PetlyaCount>parseInt(o.Constructor.PetlyaMore) & o.Constructor.PetlyaMore!=null)
						{
							PetlyaSum=(PetlyaCount-parseInt(o.Constructor.PetlyaMore))*parseFloat(o.Constructor.PetlyaSum);
						}
						else
							PetlyaSum=0;
					}
					else
						PetlyaSum=parseFloat(o.Constructor.PetlyaSum);
				Sum+=PetlyaSum;
			};
			//Навесы на рабочей створке
			if(o.Constructor.PetlyaWork==1 & RowEl.find("td[type=Construct] span workpetlya").text()!="")
			{
				var PetlyaSum=parseFloat(o.Constructor.PetlyaWorkSum);
				...
				if(PetlyaCount!=0)
					if(o.Constructor.PetlyaWorkCount==1)//Зависит от кол-ва
					{
						if(o.Constructor.PetlyaWorkMore==null)//Не заполненно поле болшльше
							PetlyaSum=PetlyaCount*parseFloat(o.Constructor.PetlyaWorkSum);
						//Заполненно поле больше, т.е. расчитываем по разнице
						if(PetlyaCount>parseInt(o.Constructor.PetlyaWorkMore) & o.Constructor.PetlyaWorkMore!=null)
						{
							PetlyaSum=(PetlyaCount-parseInt(o.Constructor.PetlyaWorkMore))*parseFloat(o.Constructor.PetlyaWorkSum);
						}
						else
							PetlyaSum=0;
					}
					else
						PetlyaSum=parseFloat(o.Constructor.PetlyaWorkSum);
				Sum+=PetlyaSum;
			};
			//Навесы на второй створке
			if(o.Constructor.PetlyaStvorka==1 & RowEl.find("td[type=Construct] span stvorkapetlya").text()!="")
			{
				var PetlyaSum=parseFloat(o.Constructor.PetlyaStvorkaSum);
				...
				if(PetlyaCount!=0)
					if(o.Constructor.PetlyaStvorkaCount==1)//Зависит от кол-ва
					{
						if(o.Constructor.PetlyaStvorkaMore==null)//Не заполненно поле болшльше
							PetlyaSum=PetlyaCount*parseFloat(o.Constructor.PetlyaStvorkaSum);
						//Заполненно поле больше, т.е. расчитываем по разнице
						if(PetlyaCount>parseInt(o.Constructor.PetlyaStvorkaMore) & o.Constructor.PetlyaStvorkaMore!=null)
						{
							PetlyaSum=(PetlyaCount-parseInt(o.Constructor.PetlyaStvorkaMore))*parseFloat(o.Constructor.PetlyaStvorkaSum);
						}
						else
						...
						PetlyaSum=parseFloat(o.Constructor.PetlyaStvorkaSum);
				Sum+=PetlyaSum;
			};
			//Ребра жесткости
			if(o.Constructor.Stiffener==1)
				if(o.Constructor.StiffenerW==1)//Зависит от кв.м.
				{
					Sum+=parseFloat(H)*parseFloat(W)*parseFloat(o.Constructor.StiffenerSum)/1000000;
				}
				else
				{
					Sum+=parseFloat(o.Constructor.StiffenerSum);
				};
			//Площадь двери
			if(o.Constructor.M2==1)
				Sum+=parseFloat(H)*parseFloat(W)*parseFloat(o.Constructor.M2Sum)/1000000;
			//Антипаника
			if(o.Constructor.Antipanik==1 & RowEl.find("td[type=Construct] span Antipanik").text()=="1")
				Sum+=parseFloat(o.Constructor.AntipanikSum);
			//Антипаника
			if(o.Constructor.Otboynik==1 & RowEl.find("td[type=Construct] span Otboynik").text()=="1")
				Sum+=parseFloat(o.Constructor.OtboynikSum);
			//Антипаника
			if(o.Constructor.Wicket==1 & RowEl.find("td[type=Construct] span Wicket").text()=="1")
				Sum+=parseFloat(o.Constructor.WicketSum);
			//Антипаника
			if(o.Constructor.BoxLock==1 & RowEl.find("td[type=Construct] span BoxLock").text()=="1")
				Sum+=parseFloat(o.Constructor.BoxLockSum);
			//Ответка
			if(o.Constructor.Otvetka==1 & RowEl.find("td[type=Construct] span Otvetka").text()=="1")
				Sum+=parseFloat(o.Constructor.OtvetkaSum);
			//Утепление
			if(o.Constructor.Isolation==1 & RowEl.find("td[type=Construct] span Isolation").text()=="1")
				Sum+=parseFloat(o.Constructor.IsolationSum);
			//Вент решетка
			if(o.Constructor.Grid==1)
			{
				...
				Sum+=o.Constructor.GridCount==1 ? CountGrid*parseFloat(o.Constructor.GridSum) : parseFloat(o.Constructor.GridSum);
			};
			...
			switch(o.Step){
				...
				case "Сварка": RowEl.find("td[type=Cost] span CostSvarka").text(Sum); break;
