	...
	switch($_POST['method'])
	{
		...
		case "PrintNaryad":
            switch (explode('.',PHP_VERSION)[0])
            {
                case "5":
                    include("../mpdf53/mpdf.php");
                    $mpdf = new mPDF('utf-8', 'A5', '8', '', 10, 10, 7, 7, 10, 10);
                    $mpdf->charset_in = 'UTF8';
                    break;
                case "7":
			...
			*/
			...
			$d=$m->query("SELECT od.id AS idDoor, n.id AS idNaryad FROM OrderDoors od, naryad n WHERE od.id=n.idDoors AND od.idOrder=$idOrder") or die($m->error);
			$arrNaryad=array();
			if($d->num_rows>0)
			    while ($r=$d->fetch_assoc())
                    $arrNaryad[]=array("idDoor"=>$r["idDoor"], "idNaryad"=>$r["idNaryad"], "flag"=>0);
			$d->close();
			...
			while($r=$d->fetch_assoc())
			{
				...
				for($NumPP=1;$NumPP<=(int)$r["Count"];$NumPP++)
				{
                    ...
                    $id=$r["id"];
                    foreach ($arrNaryad as &$n)
                        if($n["idDoor"]==$r["id"] & $n["flag"]==0){
                            $id=$n["idNaryad"];
                            $n["flag"]=1;
                            break;
                        };
					...
					$a_replace=array(
						$imgOut, $Commercie,$id, $NumInOrder, $Num.$NumPP, $r["name"] , $Size , $r["Open"], $r["Nalichnik"],$r["Dovod"],"RAL ".$r["RAL"],$r["Zamok"],$Shtild,$r["DoorNote"] ,$Svarka,$Frame,$Sborka,$Color,$Upak, $Shpt, "", "", $r["DoorAllCount"]
					);
					while(!feof($f))
