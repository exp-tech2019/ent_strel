...
$(document).ready(function () {
    ...
    //Загрзка номенклатуры о наряду
    $("#NaryadBtn").click(function () {
        SpeNaryadSelect($("#NaryadInp").val());
    })
    //Изменение состояние переключателя
    $("#DolgnostStepFilter").change(function(){
...
function SpeNaryadSelect(NaryadNum) {
    $("#GoodsTable tr").remove();
    if(NaryadNum!="")
        gl.Post(
            "TransferInEnt_SpeNaryad",
            {
                Naryad:NaryadNum
            },
            function (str) {
                var o=JSON.parse(str);
                $("#idNaryad").val(o[0].idNaryad);
                //Создадим спсиок общих idDoorGroup
                var arrDorGoups=new Array();
                var idDoorGroup=-1;
                for(let i in o)
                    switch (o[i].idDoorGroup == idDoorGroup) {
                        case true:
                            arrDorGoups.push(idDoorGroup);
                            break;
                        case false:
                            idDoorGroup = o[i].idDoorGroup;
                            break;
                    }
                ...
                for(let i in o)
                    GoodTable.AddRow(
                        ...
                        o[i].CountNaryad,
                        o[i].CountIssueOld,
                        arrDorGoups.indexOf(o[i].idDoorGroup)>-1 ? "success" : ""
                    );
                GoodTable.DolgnostStepFilter();
            }
        )
...
var GoodTable={
    AddRow:function (idGood, GoodName, Unit, StepGroup, CountStock, CountNaryad, CountIssueOld,ColorTR) {
        $("#GoodsTable").append(
            "<tr idGood='"+idGood+"' StepGroup='"+StepGroup+"' class='"+(ColorTR===undefined ? "" : ColorTR)+"'>" +
                "<td Type='GoodName'>"+GoodName+"</td> "+
                "<td Type='Unit'>"+gl.ManualUnits[Unit]+"</td>"+
                "<td Type='CountStock'>"+CountStock+"</td>"+
                "<td Type='CountNaryad'>"+CountNaryad+"</td>"+
                "<td Type='CountIssueOld'>"+CountIssueOld+"</td>"+
                "<td Type='CountIssue'><input class='form-control'> </td>"+
            "</tr>"
        );
        GoodTable.DolgnostStepFilter();
    },
    DolgnostStepFilter:function () {
        switch ($("#DolgnostStepFilter").prop("checked")){
            case true:
                switch ($("#idDolgnost").val()){
                    case "":
                        $("#GoodsTable tr").show();
                        break;
                    default:
                        var idDolgnost=$("#idDolgnost").val();
                        var Steps=new Array();
                        for(let i in DolgnostStep)
                            if(DolgnostStep[i].idDolgnost==idDolgnost) 
                                Steps.push(DolgnostStep[i].Step);
                        for(let i=0; i<$("#GoodsTable tr").length;i++){
                            var TR=$("#GoodsTable tr:eq("+i+")");
                            switch (Steps.indexOf(parseInt(TR.attr("StepGroup")))>-1){
                                case true:
                                    TR.show();
                                    break;
                                case false:
        ...
        for(var i=0; i<$("#GoodsTable tr").length;i++){
            ...
            if(TR.find("td[Type=CountIssue] input").val()!="" & TR.find("td[Type=CountIssue] input").val()!="0")
                arrTR.push({
                    ...
                    CountNaryad:TR.find("td[Type=CountNaryad]").text(),
                    CountIssueOld:TR.find("td[Type=CountIssueOld]").text(),
                    ...
                    for(var j in gd)
                        $("#GoodsDialogTable").append(
                            "<tr onclick='GoodsDialog.SelectTR(this)' idGroup='"+o[i].idGroup+"' idGood='"+gd[j].idGood+"' Step='"+o[i].Step+"'>" +
                                ...
                                "<td Type='CountStock'>"+gd[j].CountStock+"</td>"+
                            "</tr>"
                        );
                };
    ...
    "Close":function () {
        ...
        if(TR.length==1)
            GoodTable.AddRow(
                ...
                TR.attr("Step"),
                TR.find("td[Type=CountStock]").text(),
                0,
                0
            )
