<div>
    <form method="post" action="Calc.php">
        <label for="InpWith">c</label>
        <input id="InpWith" name="With">
        <label for="InpBy">по</label>
        <input id="InpBy" name="By">
        <input type="submit" value="Отправить">
    </form>
    <button onclick="EditCost()">Пересчитать</button>
</div>
...
$arrStepStr=array(
        "", "Лазер","Сгибка","Сварка","Рамка","Сборка","Покраска","Упаковка","Отгрузка","Сборка МДФ","МДФ"
);

$d = $m->query("SELECT o.Blank, DATE_FORMAT(o.BlankDate,'%d.%m.%Y') AS BlankDate, DATEDIFF(nc.DateComplite, o.BlankDate) AS DateDif, o.Shet, od.*, IF(od.S IS NOT NULL, 2, IF(od.SEqual=1, 2, 1)) AS SType, CONCAT(n.Num, n.NumPP) AS NaryadNum, nc.id AS idNC, nc.Cost, nc.Step, DATE_FORMAT(nc.DateComplite,'%d.%m.%Y') AS DateComplite, w.FIO, md.Dolgnost FROM oreders o, orderdoors od, naryad n, naryadcomplite nc, workers w, manualdolgnost md WHERE o.id=od.idOrder AND od.id=n.idDoors AND n.id=nc.idNaryad AND nc.idWorker=w.id AND w.DolgnostID=md.id AND nc.DateComplite BETWEEN STR_TO_DATE('$With','%d.%m.%Y') AND DATE_ADD(STR_TO_DATE('$By', '%d.%m.%Y'), INTERVAL 1 DAY) ORDER BY nc.Step, w.FIO") or die($m->error);
