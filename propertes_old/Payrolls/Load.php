...
$arrDoorSize=array();
$d=$m->query("SELECT * FROM payrolldoorsize_new WHERE DoorType='$TypeDoor' AND Step='$StepName'");
if($d->num_rows>0)
...
//Константы
$arrConst=array();
$d=$m->query("SELECT * FROM payrollconstant WHERE DoorType='$TypeDoor' AND Step='$StepName'");
if($d->num_rows>0)
    while ($r=$d->fetch_assoc())
        $arrConst[]=array(
            "Note"=>$r["Name"],
...
//Конструкция двери
$arrConstruct=null;
$d=$m->query("SELECT * FROM PayrollConstruct WHERE DoorType='$TypeDoor' AND Step='$StepName'");
if($d->num_rows>0)
{
    $r=$d->fetch_assoc();
    $arrConstruct=array(
