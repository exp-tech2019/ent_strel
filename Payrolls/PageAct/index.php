...
<?php
    ...
    $Sort=!isset($_POST["Sort"]) ? "Dolgnost" : $_POST["Sort"];

?>
<script>
    var idAct=<?php echo $idAct; ?>
</script>
...
<div class="panel NoPrint">
    <div class="panel-body">
        ...
        <!-- <a href="PageAct/PrintAllWorkers.php?idAct=<?php echo $idAct; ?>&Sort=<?php echo $Sort; ?>" target="_blank" class="btn btn-primary">Печать</a>-->
        <button onclick="PrintAct()" class="btn btn-primary">Печать</button>
        <script>
        	function PrintAct(){
        		var FilterStr="";
        		FilterStr+=$("#FilterFIO").val()!="" ? "&FilterFIO="+$("#FilterFIO").val() : "";
        		FilterStr+=$("#FilterDolgnost").val()!="" ? "&FilterDolgnost="+$("#FilterDolgnost").val() : "";
        		window.open("PageAct/PrintAllWorkers.php?idAct=<?php echo $idAct; ?>&Sort=<?php echo $Sort; ?>"+FilterStr,'_blank');
        	}
        </script>
...
<div class="row">
	<div class="col-xs-6">
		<div class="panel NoPrint">
		    <div class="panel-body">
		    	<form action="index.php?MVCPage=PageAct&idAct=<?php echo $idAct; ?>" method="post">
			        <label>Сортировка: </label>
			        <input type="radio" name="Sort" value="Dolgnost" <?php echo $Sort=="Dolgnost" ? "checked" : ""; ?>>
			        <label>Должность</label>
			        <input type="radio" name="Sort" value="FIO" <?php echo $Sort=="FIO" ? "checked" : ""; ?>>
			        <label>ФИО</label>
			        <input type="submit" class="btn btn-sm btn-default" value="Применить">
		    	</form>
		    </div>
		</div>
	</div>
        ...
        <table class="table table-responsive table-bordered table-hover">
            ...
            <tbody id="WorkerTable">
                <?php
                    ...
                    $d=$m->query("SELECT p.idWorker, w.FIO, m.Dolgnost, p.SumWith, p.Cost, p.SumPlus , p.NalogPercent, -1*p.SumMinus AS SumMinus FROM TempPayrollsList p, Workers w, ManualDolgnost m WHERE p.idAct=$idAct AND p.idWorker=w.id AND w.DolgnostID=m.id ORDER BY ".($Sort=="Dolgnost" ? "m.Dolgnost" : "w.FIO"));
                    while($r=$d->fetch_assoc()){
...
<div class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" id="ActionsDialog">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <button onclick="PrintOneWroker()" class="btn btn-default" style="width: 90%; margin:5px; margin-right: 0px;">Печать нарядов</button>
        </div>
    </div>
</div>
