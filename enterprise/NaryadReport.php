<?php
	...
	$d=$m->query("SELECT od.name, IF(od.S IS NOT NULL, 2, IF(od.SEqual=1, 2, 1)) AS S, nc.Cost, nc.Step FROM orderdoors od, naryad n, naryadcomplite nc 
WHERE od.id=n.idDoors AND n.id=nc.idNaryad AND 
nc.DateComplite BETWEEN STR_TO_DATE('01.01.2018','%d.%m.%Y') AND STR_TO_DATE('31.12.2018','%d.%m.%Y')") or die($m->error);
	$CostAll=0;
	$ar=array(
		array(
			"Name"=>"Одностворчатая",
	...
	foreach($ar as &$door)
		for($i=1; $i<=10; $i++)
			$door["Steps"][]=array("Step"=>$i, "Cost"=>0, "Count"=>0);
	while($r=$d->fetch_assoc())
	{
		$DoorType="";
		if(strpos($r["name"],"МДФ")!==false) $DoorType="МДФ";
		if(strpos($r["name"],"Ворота")!==false) $DoorType="Ворота";
		if(strpos($r["name"],"Люк")!==false) $DoorType="Люк";
		if($DoorType=="" & $r["S"]==1) $DoorType="Одностворчатая";
		if($DoorType=="" & $r["S"]==2) $DoorType="Двухстворчатая";
		foreach($ar as &$door)
			if($door["Name"]==$DoorType)
				foreach($door["Steps"] as &$step)
					if($step["Step"]==$r["Step"])
					{
						$step["Cost"]+=(float)$r["Cost"];
	...
	<table>
		<thead>
			<tr>
				<th>Дверь</th>
				<th colspan=2>Лазер</th>
				<th colspan=2>Гибка</th>
				<th colspan=2>Сварка</th>
				<th colspan=2>Рамка</th>
				<th colspan=2>Сборка</th>
				<th colspan=2>Покраска</th>
				<th colspan=2>Упаковка</th>
				<th colspan=2>Погрузка</th>
				<th colspan=2>МДФ</th>
				<th colspan=2>МДФ Сборка</th>
			</tr>
		</thead>
		<tbody>
			<?php
		...
		<tbody>
			...
			/*
				foreach($ard as $d)
				{
					?>
					<tr>
						<td><?php echo $d["S"]==1 ? "Одностворчатая" : "Двухстворчатая" ?>
						<?php
